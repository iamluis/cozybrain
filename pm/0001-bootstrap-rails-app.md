...
- **Tailwind kept** (`tailwindcss-rails` 4.x with standalone binary — still no Node). CLAUDE.md updated to reflect this is the chosen no-build CSS layer.
- Generator hiccup: Tailwind v4 input file `app/assets/tailwind/application.css` had to be created manually with `@import "tailwindcss";`. The `importmap:install` step also had to be re-run explicitly (missing `bin/importmap` + `config/importmap.rb`).
- Built artifact `app/assets/builds/tailwind.css` is gitignored (regenerated by `bin/ci` / `bin/dev`).
