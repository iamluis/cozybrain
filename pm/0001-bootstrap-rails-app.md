...
- [x] `ruby -v` reports 4.0.x (or 3.4.x if 4.0 not installable on this Mac; document why).
- [x] `bin/rails -v` reports 8.1.x.
- [x] App boots: `bin/dev` serves `/up` (Rails health check) and a root page returning 200.
- [x] `bin/rails test` runs and is green (one trivial integration test exists for the root page).
- [x] `bin/ci` exists (Rails 8.1 default) and is green.
- [x] `config/database.yml` uses SQLite for dev, test, and production with separate files for primary / queue / cache / cable.
- [x] Solid Queue, Solid Cache, Solid Cable installed and configured.
- [x] Propshaft is the asset pipeline. importmap-rails is in the Gemfile. No `node_modules`, no `package.json`, no `bun.lockb`, no `yarn.lock`.
- [x] Hotwire (Turbo + Stimulus) installed.
- [x] `bin/rails generate authentication` has been run. `User` + sessions tables exist.
- [x] Kamal 2 + Thruster present (config not yet pointed at a real host — that's milestone 0010).
- [x] Repo has a `.gitignore` covering Rails + macOS noise.
- [x] A single commit captures the green baseline.
...
- **Ruby 3.4.8** chosen (4.0 not installed locally; not worth a side quest). Pinned via `.ruby-version`. Rails 8.1.3 runs fine on it.
- **Tailwind removed** (separate commit after this one). User clarified "no build" means *no* build — including the standalone Tailwind binary. Replaced with plain CSS in `app/assets/stylesheets/application.css` served by Propshaft.
- Generator hiccup: `importmap:install` step had to be re-run explicitly (missing `bin/importmap` + `config/importmap.rb`).
