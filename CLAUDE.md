...
- **Asset pipeline**: Propshaft. **JS**: importmap-rails (no bundling, no transpilation). **CSS**: plain CSS in `app/assets/stylesheets/application.css`, served by Propshaft. Token-driven with CSS custom properties; semantic component classes. **NO Node, NO esbuild, NO Webpack, NO npm, NO Tailwind, NO Sass.**
